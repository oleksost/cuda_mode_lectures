...
void matmul_kernel_wmem(float* output, float* A, float* B, int width, int height, int inne_dim) {   
    ...
    extern __shared__ float mA[]; // dynamic shared memory
    float* mB = &mA[TILE_WIDTH * TILE_WIDTH];
    ...
    for (int ph = 0; ph < num_phases; ++ph) {
        ...
        if (r < height && (ph * TILE_WIDTH + tx) < inne_dim) {
            mA[ty*TILE_WIDTH+tx] = A[r * inne_dim + ph * TILE_WIDTH + tx];
        } else {
            mA[ty*TILE_WIDTH+tx] = 0.0f;
        }

        if (c < width && (ph * TILE_WIDTH + ty) < inne_dim) {
            mB[ty*TILE_WIDTH+tx] = B[(ph * TILE_WIDTH + ty) * width + c];
        } else {
            mB[ty*TILE_WIDTH+tx] = 0.0f;
        }
        ...
        for (int i = 0; i < TILE_WIDTH; ++i) {
            value += mA[ty*TILE_WIDTH+i] * mB[i*TILE_WIDTH+tx];
        }
...
torch::Tensor matmul_wmem(torch::Tensor A, torch::Tensor B) {
    ...
    matmul_kernel_wmem<<<number_of_blocks, threads_per_block, size, torch::cuda::getCurrentCUDAStream()>>>(
